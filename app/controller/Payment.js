...
class PaymentController {
  ...
  update(payment) {
    return this.payment.update(payment, { where: { cdPagamento: payment.cdPagamento } }).then(result => defaultResponse(result))
    .catch(error => errorResponse(error.message, httpStatus.UNPROCESSABLE_ENTITY));
  }

  delete(payment) {
    return this.payment.destroy({ where: { cdPagamento: payment.cdPagamento } }).then(result => defaultResponse(result))
    .catch(error => errorResponse(error.message, httpStatus.UNPROCESSABLE_ENTITY));
  }
}
